...
namespace SharpFileSystem.Tests
{
	...
	public class FileSystemPathTest
	{
		...
		static FileSystemPath fileA = FileSystemPath.Parse("/filea");
		static FileSystemPath directoryB = FileSystemPath.Parse("/directorya/directoryb/");
		static FileSystemPath fileB = FileSystemPath.Parse("/directorya/fileb.txt");
		static FileSystemPath root = FileSystemPath.Root;
		FileSystemPath fileC;

		/// <summary>
		///A test for Root
		///</summary>
		[Fact]
		public void RootTest() { Assert.Equal(root, FileSystemPath.Parse("/")); }
		...
		{
			Assert.Equal(root, directoryB.RemoveParent(directoryB));
			Assert.Equal(FileSystemPath.Parse("/fileb.txt"), fileB.RemoveParent(directoryA));
			Assert.Equal(root, root.RemoveParent(root));
			Assert.Equal(directoryB, directoryB.RemoveParent(root));
		...
		{
			Assert.Equal(directoryA, fileB.RemoveChild(FileSystemPath.Parse("/fileb.txt")));
			Assert.Equal(directoryA, directoryB.RemoveChild(FileSystemPath.Parse("/directoryb/")));
			Assert.Equal(root, directoryB.RemoveChild(directoryB));
			Assert.Equal(root, fileB.RemoveChild(fileB));
